...
output_mainDir = "plots_convergence_test" 
...
wanted_z = [6,7,8.5]  
#NOTE:need to pick out redshifts: 6, 7, and 8.5
chosen_z = redshiftSelector(wanted_z,z_arr_str[0])
print("chosen redshifts:",chosen_z) 

#NOTE: remove the if branches CAREFULLY to run all iterations
#loop over models 
            ...
            if n == 0:
                ...
                for j, cut_str in enumerate(cuts_string):
                    if j == 0:
                        ...
                        #0th: create canvas/blank plot
                        fig,ax = plt.subplots(1,1)
                        #1st: get the power spectra from maxCut (only 1 data spectrum file)
                        ...
                        k_list = np.delete(np.delete(first_data,0,1),1,1) #obj,number,axis (1 for column 0 for row)
                        print("\nk_list:",k_list)
                        print(type(k_list))
                        #NOTE: k_list and max_data may not be the same lenght!!!
