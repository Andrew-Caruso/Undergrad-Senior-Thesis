...
for m, model_dir1 in enumerate(emissivity_models1):
    ...
    original_z_all2 = z_all2.copy() #make an original copy of the unordered redshifts
    ...
    for i, cut_str1 in enumerate(cuts_string1):
        ...
        for j, k in enumerate(chosen_k):
            ...
            for n, z_str1 in enumerate(z_string1):
                ...
                z_str2 = z_string2[b] 
                #print(z_str1==z_str2," z_str:",z_str1," ",z_str2)
            ...
            z_all1 = original_z_all1.copy() #get unordered redshifts each time
            z_all2 = original_z_all2.copy() #get unordered redshifts each time
            zP_sorted1 = sorted(zip(z_all1,P_max1,P_avg1,P_stdp1,P_stdn1),key=itemgetter(0))
            zP_sorted2 = sorted(zip(z_all2,P_max2,P_avg2,P_stdp2,P_stdn2),key=itemgetter(0))
            z_all1,P_max1,P_avg1,P_stdp1,P_stdn1 = zip(*zP_sorted1)
            z_all2,P_max2,P_avg2,P_stdp2,P_stdn2 = zip(*zP_sorted2)
            #note that these are tuples not lists 
            '''
            #new data removes lowest redshift z=5.0 because no corresponding density field, thus cannot be constrained
            #the new data does not have this value while the old data does
            #therefore remove z=5.0 from the old data such that z_all1 = z_all2 = z_all
            z_all2 = list(z_all2)
            z_all2.remove(min(z_all2))
            #Is this really necessary ^ ??
            z_all2 = tuple(z_all2)
            '''
            print(z_all1 == z_all2," Z_all:",z_all1," ",z_all2)
            ...
            ax.plot(z_all1,P_max1,color='red',label="Constrained:"+maxCut1+" Mpc/h")
            ax.plot(z_all2,P_max2,color='tab:red',label="Unconstrained:"+maxCut2+" Mpc/h")
            ax.plot(z_all1,P_avg1,color='green',linestyle='dashed',label="Constrained: Average for "+cut_str1+" Mpc/h cut")
            ax.plot(z_all2,P_avg2,color='blue',linestyle='dashed',label="Unconstrained: Average for "+cut_str2+" Mpc/h cut")
            ax.fill_between(z_all1,P_stdn1,P_stdp1,alpha=0.5,color='green')
            ax.fill_between(z_all2,P_stdn2,P_stdp2,alpha=0.5,color='blue')
            ...
            ax2.plot(z_all1,P_max31,color='red',label="Constrained:"+maxCut1+" Mpc/h")
            ax2.plot(z_all2,P_max32,color='tab:red',label="Unconstrained:"+maxCut2+" Mpc/h")
            ax2.plot(z_all1,P_avg31,color='green',linestyle='dashed',label="Constrained: Average for "+cut_str1+" Mpc/h cut")
            ax2.plot(z_all2,P_avg32,color='blue',linestyle='dashed',label="Unconstrained: Average for "+cut_str2+" Mpc/h cut")
            ax2.fill_between(z_all1,P_stdn31,P_stdp31,alpha=0.5,color='green')
            ax2.fill_between(z_all2,P_stdn32,P_stdp32,alpha=0.5,color='blue')
