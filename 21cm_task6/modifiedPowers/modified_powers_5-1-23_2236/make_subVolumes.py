...
def loadIn2(directory,manifest):
    ...
    data[:,1] = [str(float(f'{float(data[i,1]):0.2f}')) for i in range(len(data[:,1]))] 
    #convert to float, truncate to 2nd decemical place and return to string
    os.chdir(current) 
    return data 
...
for m, model in enumerate(emissivity_models):
    ...
    for rs in z_arr_str[m]:
        ...
        #FIRST: need to get the std for each Z weighted by some chosen value
        #SECOND: chose subvolume max, min, step, and valid cell step
        #THIRD: loop over possible subvolumes to chose subvolumes whose density is within the cosmic density, to find all good chosen cuts
