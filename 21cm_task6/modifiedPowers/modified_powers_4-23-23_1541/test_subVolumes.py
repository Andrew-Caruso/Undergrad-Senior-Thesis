...
def loadIn2(directory,manifest):
    ...
    #convert to float, truncate to 4th decemical place and return to string
    os.chdir(current) 
    return data 
...
def contourPlotter(directory,density_list,record,model,rs_d,rs_f,N):
    ...
    return x,y,xy_d,xy_f,min_xy,max_xy,z_chosen
...
outputData_dir = '/expanse/lustre/scratch/andrewcaruso/temp_project/modified_powers/plots_densityVSfield_test/' #directory of the output data CHANGE ME
...
wanted_z = [12] #needs to be a list
...
for m, model in enumerate(emissivity_models):
    ...
    x,y,xy_d,xy_f,min_xy,max_xy,pos = contourPlotter(density_dir,density_list,record,model,z_density,z_field,N_default)
    con_f = ax[0].contourf(x,y,xy_f,cmap="jet",vmin=min_xy,vmax=max_xy)
    con_d = ax[1].contourf(x,y,xy_d,cmap="jet",vmin=min_xy,vmax=max_xy)
    ...
    ax4 = fig.add_axes([0.14,0.15,0.8,0.02]) #xmin,ymin,width,height
