...
weight = 0.01 #weight on the std 
print("std weight:",weight)
numSteps = 5 #number of both valid and invalid steps depending on the cut size N
...
#NOTE: remove all if branches CAREFULLY and also uncomment dirCreator lines 
...
for m, model in enumerate(emissivity_models):
    if m == 1:
        ...
        for q, rs in enumerate(z_arr_str[m]):
            ...
            print("\nz:",rs)
            #get the weighted std from the cosmic density 1 for the redshift z
            ...
            for N in N_chosen:
                ...
                for permTuple in perms:
                    ...
                    if avgDensity < cosmicDensity+newSTD and avgDensity > cosmicDensity-newSTD:
                        ...
                        #NOTE: uncomment this below 
                        #loadOut(newfield,longPhrase,outputCutDir) 
                        saveCount+=1
                if saveCount != len(perms):
